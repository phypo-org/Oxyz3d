...
namespace M3d {
	...
	protected:
      ...
      PP3d::Double3 lPos( cSliderPosX->value() , 	cSliderPosY->value() ,	cSliderPosZ->value() );
		...
		{
			cSliderFovY->value( cKamera.cFovY );
			//		cSliderAspect->value( cKamera.cAspect );
			cSliderZNear->value( cKamera.cZnear );
			cSliderZFar->value( cKamera.cZfar );

			cSliderSize->value(  cKamera.scale().x()  );

			cSliderPosX->value( cKamera.position().x() );
			cSliderPosY->value( cKamera.position().y() );
			cSliderPosZ->value( cKamera.position().z() );
			
			cSliderRotX->value( cKamera.angle().x() );
			cSliderRotY->value( cKamera.angle().y() );
			cSliderRotZ->value( cKamera.angle().z() );
	...
	public:
      ...
      o->end();
		...
		resetValueDialog();
			...
			{ Fl_Button* o = new Fl_Button(lW*0.5, lY, 75, 25, "Cancel");
				o->callback((Fl_Callback*)CancelCB, this );
			} // Fl_Button* o
			{ Fl_Button* o = new Fl_Button(lW*0.75, lY, 75, 25, "OK");
