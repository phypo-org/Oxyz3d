...
public:
  ...
  //--------------------------------------------------
  //---------------------------------------------------
  //void initPython(int iArgc, char *iArgv[] )
  void initPython( )
  {
    PyConfig_InitPythonConfig(&cConfig);
    
    PyWideStringList_Append( &cConfig.argv, L"Oxyd3d" );

    PyStatus lStatus = Py_InitializeFromConfig(&cConfig);

   if (PyStatus_Exception(lStatus))
      {        
        fprintf(stderr, "Cannot initialise python from arg\n");
        Py_Initialize();
      }
  }
  //--------------------------------------------------
