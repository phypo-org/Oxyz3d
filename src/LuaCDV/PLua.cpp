...
namespace PLua{
  ...
  PPTools::PSimpleMutex            *PLuaSession::sContainerLuaSessionsMutex = NULL;
  ...
  bool 
  PLuaSession::RegisterCLibrary( const char* pLibraryName, CLibraryRegisterFunction pFtn )
  {
    Init();
    sContainerCLibraryRegister->add( pLibraryName, new CLibraryRegisterFunctionStruct( pFtn) );
    return true;
  }
  //------------------------------------------------
  bool 
  PLuaSession::CmdLuaCLibraryList( ExtendIOStream& pOs, bool pLast,  CmdPList* pCmdPList, int pSecLevel )
  {
    ...
    while( lIter.hasNext() )
      {
	PPTools::PAssocType<const char*, CLibraryRegisterFunctionStruct*>::PAssocAtom *A = lIter.next();
	pOs<< A->cKey  << std::endl;
      }
  ...
  PLuaSession::Init()
  {
    if( sContainerLuaSessionsMutex == NULL && sContainerLuaSessions == NULL )
      {
	sContainerLuaSessionsMutex = new PPTools::PSimpleMutex();
	PPTools::PSimpleLocker lLock( *sContainerLuaSessionsMutex );
  ...
  //------------------------------------------------
  bool 
  PLuaSession::registerFunction( const char* pLibName, const char* pName, CLibraryFonction pFtn, int pSecurityLevel  )
  {
    PCOUT << "registerFunction " << pLibName << " "<<  pName;
    if( pSecurityLevel > cSecurityLevel )
      {
	PCOUT << " FAILED" << std::endl;
	if( cCurrentStream ){
	  err() <<  "Function register failed for :  " << pLibName << " " <<  pName << ". Security level tool low : " 
		<< pSecurityLevel << ">" << cSecurityLevel << std::endl;
	}
	return false;
      }
    PCOUT << " OK" << std::endl;
	
    PPTools::PPStringDyn pStr( pLibName );
    pStr += '_';
    pStr += pName;
